...
function createElement(){
    ...
    p1.innerHTML = "Nº de vezes em que o teclado foi acionado no campo 'Nome'"
    form.appendChild(p1)
...
_form.addEventListener("onsubmit",(evt)=>evt.preventDefault())

//3. Mostre um alerta na página quando a tela terminar de ser carregada.

function onLoad(){
    alert("3. Mostre um alerta na página quando a tela terminar de ser carregada.")
}

const label = document.querySelector('label')

// 4. Adicione uma cor a um texto quando o mouse ficar acima do mesmo e outra cor quando ele sair do mesmo.
label.onmouseover = function(){ label.classList.add('labelOnHover')}
label.onmouseout = function(){ label.classList.remove('labelOnHover')}
