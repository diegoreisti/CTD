...
SELECT idPlaylist "Id Playlist", idusuario "Id Usuário", titulo "Título", qtdcancoes "Qtd. Canções", idestado "Id Estado", Datacriacao "Data de Criação", Dataexclusao "Data de Exclusão" FROM playlist order by qtdcancoes desc limit 1;
...
alter table playlistxcancao drop FOREIGN KEY Idcancao;
alter table generoxcancao drop FOREIGN KEY cancao;
delete from playlistxcancao where idcancao in (select idcancao from generoxcancao where idgenero = 9); -- > não precisava
delete FROM cancao where idcancao in (select idcancao from generoxcancao where idgenero = 9); 
delete from generoxcancao where idgenero = 9; -- > não precisava
...
DELETE cancao FROM cancao
INNER JOIN generoxcancao ON cancao.idcancao = generoxcancao.idcancao
